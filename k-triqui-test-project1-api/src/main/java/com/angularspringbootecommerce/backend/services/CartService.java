...
public class CartService {
        ...
        if (userCart.isPresent()) {
            ...
            for (CartItemDto consolidatedCartItem : consolidatedCartItems) {
                ...
                Product product = productRepository.findById(productId)
                        .orElseThrow(() -> new AppException("Product not found", HttpStatus.NOT_FOUND, HttpStatus.BAD_REQUEST));
    ...
    public CartDto removeItemFromCart(Long userId, Long productId) {
        ...
        Cart userCart = cartRepository.findByUserId(userId)
                .orElseThrow(() -> new AppException("Cart not found", HttpStatus.NOT_FOUND, HttpStatus.BAD_REQUEST));

        CartItem cartItemToRemove = userCart.getCartItems().stream()
                .filter(item -> item.getProductId().equals(productId))
                .findFirst()
                .orElseThrow(() -> new AppException("Cart item not found", HttpStatus.NOT_FOUND, HttpStatus.BAD_REQUEST));
    ...
    public Cart getCartEntityByUserId(Long userId) {
        return cartRepository.findByUserId(userId).orElseThrow(() ->
                new AppException("Cart not found for user id: " + userId, HttpStatus.NOT_FOUND, HttpStatus.BAD_REQUEST));
    }
